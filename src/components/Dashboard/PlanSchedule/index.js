...
import { getLength, getLines, isEmptyHead, isEmptyHeadAlone, isHead, isHeadAlone, getFoot } from '../../../utils/Cell';
...
const PlanSchedule = (props) => {
  ...
  const handleSelect = ({ column, row }) => {
    setSelectedEvents((prev) => {
      if (prev.some(([column_, row_]) => column_ === column && row_ === row)) {
        return prev.filter(([col_, row_]) => column !== col_ || row_ < row || row_ > getFoot(prev, [column, row]));
      } else {
  ...
  const getEventElement = (eventsArray, i) => {
    ...
    const emptyCell = (selected, rowEnd) => (
      ...
      >
        <div className={`flex justify-end items-end w-full h-auto m-0.5 p-2 rounded ${selected ? 'border-2 border-dashed border-purple-500' : ''}`}>
          {selected && <Plus className='relative text-purple-500 p-2 hover:bg-purple-200 rounded-full transition-all box-content z-30' />}
        </div>
      </div>
    );

    const cell = (length) => (
      <div className={`flex ${classes} ${length ? `row-end-${row + length + 1}` : row + 1} cursor-not-allowed`} key={uuidv4()}>
    ...
    if (isEmptyHead(selectedEvents, column, row)) return emptyCell(true, getFoot(selectedEvents, [column, row]) + 1);

    if (!arrayInclude(selectedEvents, [column, row])) return emptyCell(false);
  };
